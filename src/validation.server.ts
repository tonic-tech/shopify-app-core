...
export const ChargeIdSchema = z.string().refine(
  (val) => {
    if (/^gid:\/\/shopify\/AppSubscription\/\d+$/.test(val)) return true;
...
export function validateChargeId(chargeId: unknown): string | null {
 ...
 * ```
 */
export function createPlanValidator<T extends string>(validPlans: readonly T[]) {
  type Plan = T | "FREE";

  function validatePlan(planName: unknown): Plan {
    ...
    if (validPlans.includes(upperPlan as T)) {
      return upperPlan as T;
    }
...
export function validateSubscriptionPayload(payload: unknown) {
...
export function validateGDPRPayload(payload: unknown) {
